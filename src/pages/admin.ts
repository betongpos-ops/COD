...
window.deleteItem = async function (trackingNo: string) {
  const ok = await Swal.fire({ title: 'ยืนยันการลบ?', icon: 'warning', html: `<span style="font-family:monospace;">${trackingNo}</span>`, showCancelButton: true, confirmButtonColor: '#ef3e25', confirmButtonText: 'ลบเลย', cancelButtonText: 'ยกเลิก' })
  if (!ok.isConfirmed) return
...
window.deleteAllToday = async function () {
  ...
  const ok = await Swal.fire({
    ...
    confirmButtonColor: '#ef3e25', confirmButtonText: 'ล้างข้อมูล', cancelButtonText: 'ยกเลิก',
...
window.generateReport = async function () {
  ...
  const logoHtml  = logoSrc ? `<img src="${logoSrc}" style="height:36pt;width:auto;max-width:220pt;object-fit:contain;display:block;margin-bottom:6pt;" alt="ไปรษณีย์ไทย">` : '<div style="font-size:14pt;font-weight:700;color:#002169;">ระบบจัดการพัสดุคงค้าง COD</div>'
  ...
  const colHd = `<tr style="background:#002169;color:#fff;"><th style="${TH}text-align:center;width:28px;">ที่</th><th style="${TH}width:138px;">Tracking No.</th><th style="${TH}width:62px;">ปลายทาง</th><th style="${TH}">สาเหตุล่าสุด</th><th style="${TH}text-align:center;width:46px;">ค้าง</th><th style="${TH}text-align:center;width:88px;">สถานะชิ้นงาน</th></tr>`

  function buildRows(arr: Parcel[]) {
    return arr.map((d, i) => {
      const noItem = d.image_url === 'NO_ITEM'; const hasPhoto = d.image_url && !noItem
      const bg = i % 2 === 0 ? '#fff' : '#f8fafc'; const stBg = noItem ? '#fff1ef' : hasPhoto ? '#f0fdf4' : '#fefce8'
      const stCl = noItem ? '#d4361e' : hasPhoto ? '#15803d' : '#92400e'; const stTx = noItem ? '✗ ไม่มีชิ้นงาน' : hasPhoto ? '✓ มีชิ้นงาน' : '⋯ ยังไม่ตรวจสอบ'
  ...
  const sectHtml = ops.map(op => {
    ...
    const s14 = d14.length ? `<div style="background:#e8ecf5;padding:4px 10px;border-left:4px solid #002169;font-size:8.5pt;font-weight:700;color:#001752;">▸ COD 1-4 วัน (${d14.length} รายการ | มีชิ้นงาน: ${has14} | ไม่มีชิ้นงาน: ${no14})</div><table style="width:100%;border-collapse:collapse;">${colHd}${buildRows(d14)}</table>` : ''
    const s5p = d5p.length ? `<div style="background:#ffd5cf;padding:4px 10px;border-left:4px solid #ef3e25;font-size:8.5pt;font-weight:700;color:#c83015;${d14.length ? 'margin-top:8pt;' : ''}">▸ COD 5+ วัน (${d5p.length} รายการ | มีชิ้นงาน: ${has5p} | ไม่มีชิ้นงาน: ${no5p})</div><table style="width:100%;border-collapse:collapse;">${colHd}${buildRows(d5p)}</table>` : ''
    return `<div style="margin-bottom:14pt;page-break-inside:avoid;"><div style="background:#002169;color:#fff;padding:5px 10px;display:flex;justify-content:space-between;align-items:center;"><span style="font-size:10.5pt;font-weight:700;">&#128100; ${op}</span><span style="font-size:8.5pt;">&#10003; มีชิ้นงาน: ${has14 + has5p} | &#10007; ไม่มีชิ้นงาน: ${no14 + no5p} | รวม: ${items.length}</span></div>${s14}${s5p}</div>`
  }).join('')
  ...
  const html = `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><title>รายงาน COD</title><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;500;600;700&display=swap" rel="stylesheet"><style>@page{size:A4;margin:14mm 12mm 18mm;}*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Sarabun',sans-serif;font-size:10pt;color:#111827;background:#fff;}@media print{.no-print{display:none!important;}}.toolbar{background:#002169;color:#fff;padding:10px 18px;display:flex;align-items:center;gap:10px;position:sticky;top:0;z-index:99;}.btn-print{background:#ef3e25;color:#fff;border:none;padding:8px 20px;border-radius:6px;font-size:14px;cursor:pointer;font-family:inherit;font-weight:600;}.btn-close{background:rgba(255,255,255,.15);color:#fff;border:1px solid rgba(255,255,255,.3);padding:8px 14px;border-radius:6px;font-size:14px;cursor:pointer;font-family:inherit;}.wrap{padding:2pt 4pt;}</style></head><body><div class="no-print toolbar"><span style="flex:1;font-size:15px;font-weight:700;">รายงาน COD — ตัวอย่างก่อนพิมพ์</span><button class="btn-print" onclick="window.print()">🖨 พิมพ์ / บันทึก PDF</button><button class="btn-close" onclick="window.close()">✕ ปิด</button></div><div class="wrap"><div style="border:2px solid #002169;padding:8pt 12pt;margin-bottom:12pt;"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12pt;"><div>${logoHtml}<div style="font-size:11pt;font-weight:600;margin-top:3pt;">รายงาน: พัสดุ COD ที่มีอยู่จริง (Actual COD Report)</div><div style="font-size:8.5pt;color:#4b5563;margin-top:3pt;">ที่ทำการไปรษณีย์: ${session.branch_name}</div></div><div style="text-align:right;flex-shrink:0;"><div style="font-size:8.5pt;color:#4b5563;">วันที่พิมพ์</div><div style="font-size:10pt;font-weight:600;">${printDt}</div><div style="font-size:8.5pt;color:#4b5563;margin-top:4pt;">ผู้ดำเนินการ: <b>${ops.length}</b> คน | รวม: <b>${allData.length}</b> ชิ้น</div></div></div><div style="margin-top:7pt;display:flex;gap:8pt;flex-wrap:wrap;"><span style="background:#f0fdf4;color:#15803d;padding:3pt 8pt;border-radius:3pt;font-size:9pt;font-weight:600;">✓ มีชิ้นงาน: ${gtHas}</span><span style="background:#fff1ef;color:#d4361e;padding:3pt 8pt;border-radius:3pt;font-size:9pt;font-weight:600;">✗ ไม่มีชิ้นงาน: ${gtNo}</span><span style="background:#fefce8;color:#92400e;padding:3pt 8pt;border-radius:3pt;font-size:9pt;font-weight:600;">⋯ ยังไม่ตรวจสอบ: ${allData.length - gtHas - gtNo}</span></div></div>${sectHtml}<div style="margin-top:14pt;page-break-inside:avoid;"><div style="background:#002169;color:#fff;padding:5pt 10pt;font-size:10pt;font-weight:700;">สรุปรายงาน — จำแนกตามผู้ดำเนินการ</div><table style="width:100%;border-collapse:collapse;"><tr style="background:#374151;color:#fff;"><th style="${THS}" rowspan="2">ผู้ดำเนินการ</th><th style="${THS}" colspan="3">COD 1-4 วัน</th><th style="${THS}" colspan="3">COD 5+ วัน</th><th style="${THS}" colspan="3">รวมทั้งหมด</th></tr><tr style="background:#4b5563;color:#fff;"><th style="${THS}font-size:8pt;">มีชิ้นงาน</th><th style="${THS}font-size:8pt;">ไม่มีชิ้นงาน</th><th style="${THS}font-size:8pt;">รวม</th><th style="${THS}font-size:8pt;">มีชิ้นงาน</th><th style="${THS}font-size:8pt;">ไม่มีชิ้นงาน</th><th style="${THS}font-size:8pt;">รวม</th><th style="${THS}font-size:8pt;">มีชิ้นงาน</th><th style="${THS}font-size:8pt;">ไม่มีชิ้นงาน</th><th style="${THS}font-size:8pt;">รวม</th></tr>${sumBody}<tr style="background:#002169;color:#fff;font-weight:700;"><td style="${TD2}border-color:#374151;">รวมทั้งหมด</td><td style="${TD2}border-color:#374151;text-align:center;">${gT14h}</td><td style="${TD2}border-color:#374151;text-align:center;">${gT14n}</td><td style="${TD2}border-color:#374151;text-align:center;">${gT14t}</td><td style="${TD2}border-color:#374151;text-align:center;">${gT5h}</td><td style="${TD2}border-color:#374151;text-align:center;">${gT5n}</td><td style="${TD2}border-color:#374151;text-align:center;">${gT5t}</td><td style="${TD2}border-color:#374151;text-align:center;">${gtHas}</td><td style="${TD2}border-color:#374151;text-align:center;">${gtNo}</td><td style="${TD2}border-color:#374151;text-align:center;">${gT14t + gT5t}</td></tr></table></div></div></body></html>`
...
window.generateUrgentReport = async function () {
  ...
  const logoSrc2  = await getLogoDataUrl()
  const logoHtml2 = logoSrc2 ? `<img src="${logoSrc2}" style="height:28pt;width:auto;max-width:180pt;object-fit:contain;display:block;" alt="ไปรษณีย์ไทย">` : ''
  const TH = 'padding:5px 7px;border:1.5px solid #374151;font-size:8.5pt;text-align:center;background:#002169;color:#fff;white-space:nowrap;vertical-align:middle;'; const TD = 'padding:4px 7px;border:1px solid #6b7280;font-size:9pt;vertical-align:middle;'
  ...
  const sumHtml = Object.entries(opSummary).sort(([a], [b]) => a.localeCompare(b, 'th')).map(([op, cnt], i) => `<tr style="background:${i % 2 === 0 ? '#fff' : '#fef9f0'};"><td style="${TDS}text-align:center;">${i + 1}</td><td style="${TDS}font-weight:500;">${op}</td><td style="${TDS}text-align:center;font-weight:700;color:#d4361e;font-size:11pt;">${cnt}</td><td style="${TDS}text-align:center;color:#6b7280;font-size:8.5pt;">ชิ้น</td></tr>`).join('')
